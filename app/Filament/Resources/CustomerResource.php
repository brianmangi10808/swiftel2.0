...
public static function form(Schema $schema): Schema
{
    ...
    ->required(),
                            ...
                            Forms\Components\DateTimePicker::make('expiry_date')
                                ->label('Expiry Date')
                                   ->hidden(fn () => in_array(Auth::id(), [39, 35, 30, 29,42])) 
                               ->required()
...
Tabs\Tab::make('Activity Logs')
    ->icon('heroicon-m-clipboard-document-list')
    ->schema([
        Forms\Components\Placeholder::make('activity_logs_tab')
            ->label('')
            ->content(fn ($record) => view('filament.tables.customer-activity-logs', [
                'logs' => $record
                    ? \App\Models\ActivityLog::where('model', 'App\\Models\\Customer')
    ...
    ->getStateUsing(fn ($record) => 
        $record->created_at && $record->created_at->isAfter(now()->subMonth())
            ? 'New' 
            : ''
    )
    ->color(fn ($record) => 
       ...
       ->weight(FontWeight::Bold),
                ...
                Tables\Columns\TextColumn::make('credit')->label('Credit Balance')->sortable(),
                 ...
                 Tables\Columns\TextColumn::make('expiry_date')->dateTime(),
                  Tables\Columns\TextColumn::make('Calling_Station_Id')
    ->label('Mac Address')
    ->searchable(),
...
Tables\Filters\Filter::make('active_no_payment')
    ...
    ->query(function (Builder $query): Builder {
        return $query
            ...
            ->whereNotExists(function ($subQuery) {
                $subQuery->select('id')
                    ->from('payments')
                    ->whereColumn('payments.bill_ref_number', 'customers.username')
                    ->where('payments.created_at', '>=', now()->subDays(32));
            });
    }),
...
Actions\BulkActionGroup::make(array_merge(
    ...
    ->form([
            ...
            ->content(
                new \Illuminate\Support\HtmlString('
                    <div class="text-sm text-gray-500 flex flex-wrap gap-2">
                        <span class="bg-gray-100 dark:bg-gray-700 rounded px-2 py-1 font-mono">{firstname}</span>
                        <span class="bg-gray-100 dark:bg-gray-700 rounded px-2 py-1 font-mono">{lastname}</span>
                        <span class="bg-gray-100 dark:bg-gray-700 rounded px-2 py-1 font-mono">{username}</span>
                        <span class="bg-gray-100 dark:bg-gray-700 rounded px-2 py-1 font-mono">{mobile_number}</span>
                        <span class="bg-gray-100 dark:bg-gray-700 rounded px-2 py-1 font-mono">{credit}</span>
                        <span class="bg-gray-100 dark:bg-gray-700 rounded px-2 py-1 font-mono">{expiry_date}</span>
                        <span class="bg-gray-100 dark:bg-gray-700 rounded px-2 py-1 font-mono">{status}</span>
                        <span class="bg-gray-100 dark:bg-gray-700 rounded px-2 py-1 font-mono">{email}</span>
                        <span class="bg-gray-100 dark:bg-gray-700 rounded px-2 py-1 font-mono">{service}</span>
                        <span class="bg-gray-100 dark:bg-gray-700 rounded px-2 py-1 font-mono">{group}</span>
                        <span class="bg-gray-100 dark:bg-gray-700 rounded px-2 py-1 font-mono">{sector}</span>
                    </div>
                    <p class="mt-2 text-xs text-gray-400">Click a placeholder to copy, then paste it into your message.</p>
                ')
            ),
        ...
        Forms\Components\Textarea::make('message')
            ...
            ->rows(4)
            ->maxLength(320)
            ->helperText('Use placeholders above to personalize each message. E.g: Dear {firstname}, your account expires on {expiry_date}.')
            ->live()
            ->afterStateUpdated(function ($state, Forms\Set $set) {
                // optionally count characters live
            }),
        ...
        Forms\Components\Placeholder::make('char_count')
            ...
            ->content(function (Forms\Get $get) {
                $msg = $get('message') ?? '';
                $len = strlen($msg);
                $parts = $len > 0 ? ceil($len / 160) : 0;
                $color = $len > 320 ? 'text-red-500' : ($len > 160 ? 'text-yellow-500' : 'text-green-600');
                return new \Illuminate\Support\HtmlString(
                    "<span class='text-sm font-medium {$color}'>{$len} characters / {$parts} SMS part(s)</span>"
                );
            }),
    ])
   ->action(function (Collection $records, array $data) {

    $gateway = \App\Models\SmsProvider::where('active', true)->first();

    if (!$gateway) {
        Notification::make()
            ->title('No active SMS Provider found')
            ->body('Please configure and activate an SMS provider in settings.')
            ->danger()
            ->send();
        return;
    }

    // Define the resolver as a local closure — no self:: needed
    $resolvePlaceholders = function (string $message, $customer): string {
        return str_replace(
            [
                '{firstname}',
                '{lastname}',
    ...
    foreach ($records as $customer) {
        ...
        $personalizedMessage = $resolvePlaceholders($data['message'], $customer);

        try {
            $response = Http::timeout(15)->post($gateway->api_url, [
            ...
            if ($response->successful()) {
                ...
                $failed++;
                \Log::warning("SMS failed for customer [{$customer->id}]: " . $response->body());
            }

        } catch (\Exception $e) {
            $failed++;
            \Log::error("SMS exception for customer [{$customer->id}]: " . $e->getMessage());
        }

       \App\Models\Messages::create([
            'company_id'   => $customer->company_id,  // ← from customer record
    ...
    Notification::make()
        ...
        ->body(implode(' | ', array_filter([
            ...
            $skipped ? " Skipped (no number): {$skipped}" : null,
        ])))
        ->when($failed === 0 && $skipped === 0, fn ($n) => $n->success())
        ->when($failed > 0 && $sent > 0,        fn ($n) => $n->warning())
        ->when($sent === 0,                     fn ($n) => $n->danger())
        ->persistent()
        ->send();
